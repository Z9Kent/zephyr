...
#include "Z9Crypto.h"       // key negotiation
...
KCB& gen_boardInfo(uint8_t& seq_number)
{
    ...
    struct 
    {
        uint8_t p0 = Z9IO_PROTOCOL_VERSION_MAJOR;
        uint8_t d1 = Z9IO_PROTOCOL_VERSION_MINOR;
        uint8_t m0 = 20;                             // model #
        uint8_t fw[2] = {0, 1};                           // firmware: major/minor
        uint8_t en;
        uint8_t sn[8] = {0x88,0x77,0x66,0x55,0x11,0x22,0x33,0x44 };
        uint8_t z0[17] = {};
    } data;

    data.en = is_encrypted;

    return gen_message(seq_number, Hello_DISCRIMINATOR, sizeof(data), (uint8_t *)&data);
}
